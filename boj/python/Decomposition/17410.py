...
class SqrtDecomposition:
    ...
    def update(self, index, k):
        ...
        target = bisect_left(self.bucket[group], k)
    ...
    def query(self, left, right, k):
        ...
        right_start = right_group * self.sqrt - 1
        for x in range(right, right_start, -1):
            result += self.arr[x] > k

        for x in range(left_group+1, right_group):
            result += len(self.bucket[x]) - bisect_right(self.bucket[x], k)
